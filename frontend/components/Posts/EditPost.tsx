...
export default function EditPost({ id }: { id: string }) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Edit post image preview URL from the file
    const url = URL.createObjectURL(file);
    setPreview(url);
  };

  const handleEdit = async (data: PostType, published: boolean) => {
    setError(null);
    try {
      ...
      <div className="flex flex-col  mb-6 m-4">
        <div className="flex items-center justify-between mb-8">
          <div className="flex flex-col items-start">
            <h1 className="text-4xl font-semibold font-serif">Edit Post</h1>
          ...
          <div className="flex gap-8">
            <button
              ...
              onClick={handleSubmit((data) => handleEdit(data, false))}
            ...
            <button
              ...
              disabled={isSubmitting}
              aria-busy={isSubmitting}
              onClick={handleSubmit((data) => handleEdit(data, true))}
        ...
        <div>
          <form>
            ...
            <div className="mb-4 w-full flex flex-col items-start">
              ...
              <label
                htmlFor="image"
                className="cursor-pointer w-full h-64 border-2 border-dashed bg-gray-50 border-gray-300 rounded-md flex items-center justify-center overflow-hidden"
              >
                {preview ? (
                  <Image
                    src={preview}
